...
int rearIn1 = 7; int rearIn2 = 12; int enable1 = 6;

int steeringIn1 = 2; int steeringIn2 = 4; int enable2 = 3;

int num = 1; int k=0; int p=2;

float threshold = 25;

float motionX = 0; float motionY = 0;

float lerpX = 0; float lerpY = 0;

long interval = 1000; long previousMillis = 0;

int ledPin = 13;

void setup() { size(352, 288); String[] cameras = Capture.list(); printArray(cameras); video = new Capture(this, 352, 288, 30); video.start(); prev = createImage(352, 288, RGB);

println(Arduino.list()); arduino = new Arduino(this, Arduino.list()[0], 57600); arduino.pinMode(ledPin, Arduino.OUTPUT);

arduino.pinMode(rearIn1, Arduino.OUTPUT); arduino.pinMode(rearIn2, Arduino.OUTPUT); arduino.pinMode(enable1, Arduino.OUTPUT);

arduino.pinMode(steeringIn1, Arduino.OUTPUT); arduino.pinMode(steeringIn2, Arduino.OUTPUT); arduino.pinMode(enable2, Arduino.OUTPUT);
